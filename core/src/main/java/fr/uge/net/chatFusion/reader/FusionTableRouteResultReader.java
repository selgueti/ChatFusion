...
public class FusionTableRouteResultReader implements Reader<FusionTableRouteResult> {
    ...
    @Override
    public FusionTableRouteResult get() {
        if (state != State.DONE) {
            throw new IllegalStateException();
        }
        return new FusionTableRouteResult(nbMembers, routes);
    }
